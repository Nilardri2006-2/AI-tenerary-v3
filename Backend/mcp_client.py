...
class Dest(BaseModel):
    dest:str = Field(
        description="The destination city or country. Return only the destination name."
    )
...
User query: {query}
Rules:
- Return only the destination city or country.
- No explanation.
- No extra words.
"""
    dest_llm = llm.with_structured_output(Dest)

    response = dest_llm.invoke(prompt)
